...
<table style="width: 100%">
    <tr>
        <th style="width: 33%; text-align: left; text-decoration: underline;">Order Number</th>
        <th style="width: 33%; text-align: left; text-decoration: underline;">Address</th>
        <th style="width: 33%; text-align: left; text-decoration: underline;">Estimated Delivery</th>
    </tr>
    <tr>
        <td>Inv-<?= date('y') ?>1000<?= $order->order_id ?></td>
        <td><?= $order->order_shipping_address ?><br>Phone: <?= $user->user_mobile ?></td>
        <td><?= date('d M, Y', (strtotime('+2 day', strtotime(date("Y-m-d"))))) ?></td>
    </tr>
</table>
<br><br>
<table style="width: 100%">
    ...
    <tr>
        <td><?= $counter ?>. <?= $item->order_product_name ?></td>
        <td><?= $item->order_product_quantity ?></td>
        <td>৳ <?= $item->order_product_total_price ?></td>
    </tr>
    <?php $counter++; } ?>
