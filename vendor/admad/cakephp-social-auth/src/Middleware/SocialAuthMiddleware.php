...
class SocialAuthMiddleware
{
    ...
    protected function _getUser($providerName, ServerRequest $request)
    {
        ...
        $profile = $this->_userModel->find()->where(["email" => $identity->email])->first();
    ...
    protected function _patchProfile(
        ...
        if ($profile === null) {
            ...
            // ]);
            $profile = $this->_userModel->newEntity();
        }
        ...
        foreach (get_object_vars($identity) as $key => $value) {
            switch ($key) {
                ...
                case 'picture':
                    if(is_object($value)){
                        $data['user_profile_image'] = $value->data->url;
                    }else{
                        $data['user_profile_image'] = $value;
                    }
                    break;
                default:
                    $data[$key] = $value;
        ...
        $data['name_slug'] = strtolower(str_replace(" ","_",$data['name']));
        $data['user_type'] = 'guest';
        $data['user_role'] = 'customer';
        $data['password'] = '123456';
        $data['user_mobile'] = substr(str_shuffle(str_repeat("abcdefghijklmnopqrstuvwxyz", 11)), 0, 11);
        $data['user_profile_image'] = 'male-avater.png';
        $data['deleted'] = 0;
        $data['login_status'] = 1;
        $hash = sha1($data['name'] . rand(0, 100));
        $data['remember_token'] = $hash;
        $data['created_at'] = Time::now();
        $data['updated_at'] = Time::now();

        // return $this->SocialProfiles->patchEntity($profile, $data);
        return $this->_userModel->patchEntity($profile, $data);
    }
    ...
    {
        if (!$this->_userModel->save($profile)) {
